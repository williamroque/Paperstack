...
class HeaderElement(u.WidgetWrap):
    "The Widget corresponding to the table header."
    ...
    def __init__ (self):
        ...
        column_num = min(width // COLUMN_WIDTH, len(COLUMNS))
        remainder = width - column_num * COLUMN_WIDTH

        column_width = COLUMN_WIDTH + remainder // column_num
        column_ratio = column_width / width * 100
...
class RowElement(u.WidgetWrap):
    ...
    def __init__ (self, record, keymap):
        ...
        for i in range(column_num):
            column = record.record[COLUMNS[i].lower()]
